...
function getIsSender(address: string, transaction: any): boolean {
  return transaction.sender === address;
}

function getGas(transaction: any): bigint {
...
function AmountAfterCell({balanceChange}: BalanceChangeCellProps) {
  return (
    <GeneralTableCell sx={{textAlign: "right"}}>
      {balanceChange.amountAfter.toString()}
    </GeneralTableCell>
  );
}
